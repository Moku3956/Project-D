...
**将来この型チェックが実装されても壊れないようにする(ユーザー確定済み)。** パディング長とデモ用テーブルの`VARCHAR`宣言を同じ定数(`NAME_COLUMN_LENGTH`)から作ることで、`name VARCHAR(700)`のようにテーブル自体をパディング後の長さで宣言している。`VARCHAR(50)`のような、実際に入れるデータより小さい宣言はしない。project_issuesの「型チェック未実装」がいつか直っても、このアプリの初期化SQLは壊れない。
